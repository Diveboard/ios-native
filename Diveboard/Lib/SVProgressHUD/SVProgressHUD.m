...
+ (void)showWithStatus:(NSString *)status maskImage:(UIImage *)maskImage{
    
    [self sharedView].maskImageView.image = maskImage;
    [[self sharedView] showProgress:-1 status:status maskType:SVProgressHUDMaskTypeImage];
}
+ (void)showWithMaskImage:(UIImage *)maskImage{
    
    [self sharedView].maskImageView.image = maskImage;
    
    [[self sharedView] showProgress:-1 status:nil maskType:SVProgressHUDMaskTypeImage];
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        ...
        case SVProgressHUDMaskTypeImage :{
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    [self setFrame:orientationFrame];
    [self.maskImageView setFrame:orientationFrame];
